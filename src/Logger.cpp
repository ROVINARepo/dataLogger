...
#include <g2o_frontend/g2o_sensor_data/SensorData.h>
...
void writeQueue() {
  ...
  // this is the vertex where we are apcking the data
  ...
  while (true) {
    if(! _queue.empty()) {  
      ...
      if (timeNow - data->timeStamp()> initialDelay)  { // we have enough stuff in the queue
	_queue.pop_front();
	if (! nptr->ok())
	  continue;
...
int main(int argc, char** argv) {
  ...
  kinect->setTopics("/camera/rgb/image_mono","/camera/depth_registered/image_raw");
  kinect->getParameter()->setId(0);
